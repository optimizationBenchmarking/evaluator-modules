...
public class AllAggregation2DLogarithmic extends ExperimentSetModuleTest {

  /** aggregate things linearly */
  public AllAggregation2DLogarithmic() {
  ...
  protected Configuration getConfiguration(final IExperimentSet data) {
    ...
    config.get(FunctionAttribute.X_AXIS_PARAM, dimParser,
        dimParser.parseString("ln(|" + dimX.getName() + //$NON-NLS-1$
            "|+1)")); //$NON-NLS-1$
    ...
    config.get(Aggregation2D.PRIMARY_AGGREGATE_PARAM,
        ...
        AllAggregation2DLogarithmic._getAggregate(random));
